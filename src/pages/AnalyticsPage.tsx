    ...
    <div className="container max-w-4xl py-10">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <h2 className="mt-8 text-sm font-semibold text-foreground">Perspective engagement</h2>
          <p className="mt-1 text-xs text-muted-foreground">
            Explicit switches away from the current selection ({switchTotal} recorded).
          </p>
          {perspectiveCounts.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              No perspective switches recorded yet — everyone is still on the default.
            </p>
          ) : (
            <div className="mt-3 space-y-1.5">
              {perspectiveCounts.map(([id, count]) => (
                <div key={id} className="rounded-md border border-border bg-card px-4 py-2">
                  <div className="flex items-center justify-between gap-3 text-xs">
                    <span className="font-medium capitalize text-foreground">
                      {id.replace(/-/g, " ")}
                    </span>
                    <span className="text-muted-foreground">
                      {count} ({Math.round((count / switchTotal) * 100)}%)
                    </span>
                  </div>
                  <div className="mt-1.5 h-1.5 rounded-full bg-muted">
                    <div
                      className="h-1.5 rounded-full bg-primary"
                      style={{ width: `${(count / switchTotal) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
